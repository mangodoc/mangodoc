# 更新日志
## 2026-06-14
1. 发布 v2.14.2，修复读取评论时无效 token 导致 GitHub API 返回 401。
   - 读取评论不再发送 token（公开仓库无需认证），仅写入时使用。
2. 发布 v2.14.1，修复悬浮 TOC 点击最后一个书签时顶部导航条消失的问题。
...
2. 修复 `getActiveIndexFromHash()` 只检查首个菜单项导致高亮错误。
3. 修复 `updateActiveMenu()` 在 Vue 编译后 `index` 属性丢失无法获取索引。
4. 修复首次加载菜单高亮时机过早（Vue 编译前执行）。
5. 新增 `popstate` 监听，修复浏览器前进/后退时菜单不更新。
6. 修复上/下页导航出现在最近修改时间下方，改为内容区与修改时间之间。
7. 发布 v2.7.1，新增上/下页导航插件 + 修复多项导航与封面页问题。
...
11. 新增上/下页导航插件 `pagenav.ts`，基于侧栏顺序在文档底部显示上一页/下一页，内置三款主题均支持。

## 2024-03-10
1. 发布2.5.0，新增内置插件iconfont，支持iconfont字体图标。
2. 内置常用图标（code、log、home、search等），支持自定义iconfont资源。
3. 更新demo示例，添加iconfont使用案例。

## 2024-03-08
1. 修复刷新页面后，网页标题名称显示不正确的问题。
2. 升级mango-valine版本到1.2.2，修复刷新首次进入页面评价加载不出来的问题。
3. 修复guide中的cover里的图片链接问题。
...
## 2024-02-24
1. 发布2.2.3，新增内置插件pageconfig,支持页面配置。
